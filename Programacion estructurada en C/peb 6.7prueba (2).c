...
int main() 
{
    ...
    while (cod != 0) 
    {
            if (buscar(archstock,cod,&prod)==1) 
            {
                ...
                else 
                 {
                    ...
                    while (fread(&faltante_prod, sizeof(faltante_prod), 1, archfalta) == 1 && registro_existe==0) 
                    {
                        if (faltante_prod.codart == cod) 
                        {
                           ...
                           fseek(archfalta, -sizeof(faltante_prod), SEEK_CUR);
...
int buscar(FILE *archivo,int codigoabuscar,Sprod *prod)
{
 int encontrar;
 ...
 encontrar=0;
while (fread(prod, sizeof(*prod), 1, archivo) == 1 && encontrar==0) 
{
 
       if (prod->codart == codigoabuscar) 
        {
          encontrar = 1;
          }
     }
